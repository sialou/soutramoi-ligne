...
@Component({
  selector: 'app-froid',
  templateUrl: './froid.component.html',
  styleUrls: ['./froid.component.css']
})
export class FroidComponent {

  signupFroidForm: FormGroup = new FormGroup({
  ...
  ngOnInit(): void {
    this.signupFroidForm = this.fb.group({
  ...
  async onSubmit(){
    ...
    let response = await emailjs.send("service_l2tmp06","template_6m5ihd4",{
      from_name: this.signupFroidForm.value.name,
      to_name: "Froid",
      from_email: " ",
      subject: "Prestataire_subject",
      message:'Service : ' +this.signupFroidForm.value.service +'Ville : ' +this.signupFroidForm.value.ville +'Commune :'+this.signupFroidForm.value.commune+ 'Teléphone :'+this.signupFroidForm.value.tel ,
      });
      this.formSubmitted = true;
      this.signupFroidForm.reset();
    ...
    console.log(this.signupFroidForm.value);*/
    } else {
