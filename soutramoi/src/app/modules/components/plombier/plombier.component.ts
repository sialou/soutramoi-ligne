...
export class PlombierComponent {
  signupPlombierForm: FormGroup = new FormGroup({
  ...
  ngOnInit(): void {
    this.signupPlombierForm = this.fb.group({
  ...
  async onSubmit(){
    ...
    let response = await emailjs.send("service_l2tmp06","template_6m5ihd4",{
      from_name: this.signupPlombierForm.value.name,
      to_name: "plombierie",
      from_email: " ",
      subject: "Prestataire_subject",
      message:'Service : ' +this.signupPlombierForm.value.service +'Ville : ' +this.signupPlombierForm.value.ville +'Commune :'+this.signupPlombierForm.value.commune+ 'Teléphone :'+this.signupPlombierForm.value.tel ,
      });
      this.formSubmitted = true;
      this.signupPlombierForm.reset();
    ...
    console.log(this.signupPlombierForm.value);*/
    } else {
