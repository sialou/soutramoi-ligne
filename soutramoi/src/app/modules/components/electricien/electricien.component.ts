...
export class ElectricienComponent {
  ...
  signupElectricienForm: FormGroup = new FormGroup({
  ...
  ngOnInit(): void {
    this.signupElectricienForm = this.fb.group({
  ...
  async  onSubmit(){
    ...
    let response = await emailjs.send("service_l2tmp06","template_6m5ihd4",{
      from_name: this.signupElectricienForm.value.name,
      to_name: "Electicité",
      from_email: " ",
      subject: "Prestataire_subject",
      message:'Service : ' +this.signupElectricienForm.value.service +'Ville : ' +this.signupElectricienForm.value.ville +'Commune :'+this.signupElectricienForm.value.commune+ 'Teléphone :'+this.signupElectricienForm.value.tel ,
      });
      this.formSubmitted = true;
      this.signupElectricienForm.reset();
    ...
    console.log(this.signupElectricienForm.value);*/
    } else {
