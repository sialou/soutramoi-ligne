...
declare var M: any;
@Component({
  selector: 'app-peintre',
  templateUrl: './peintre.component.html',
  styleUrls: ['./peintre.component.css']
})
export class PeintreComponent {

  signupPeintreForm: FormGroup = new FormGroup({
  ...
  ngOnInit(): void {
    this.signupPeintreForm = this.fb.group({
  ...
  async onSubmit(){
    ...
    let response = await emailjs.send("service_l2tmp06","template_6m5ihd4",{
      from_name: this.signupPeintreForm.value.name,
      to_name: "peintre",
      from_email: " ",
      subject: "Prestataire_subject",
      message:'Service : ' +this.signupPeintreForm.value.service +'Ville : ' +this.signupPeintreForm.value.ville +'Commune :'+this.signupPeintreForm.value.commune+ 'Teléphone :'+this.signupPeintreForm.value.tel ,
      });
      this.formSubmitted = true;
      this.signupPeintreForm.reset();
    ...
    console.log(this.signupPeintreForm.value);*/
    } else {
